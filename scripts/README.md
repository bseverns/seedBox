...
| `check_musical_effect_contract.py` | Checks a native input probe summary against `docs/fixtures/external_input_scenarios.json`. | Fails dry, silent, clipped, stereo-flat, temporally-flat, or under-reseeded renders according to the scenario's conservative contract. |
...
| `smoke_native_input_probe.py` | Builds and smoke-tests `seedbox_native_input_probe` with a generated tiny WAV. | Writes scratch input/output/status/summary files under `build/native_input_probe_smoke/`, runs `granular-live`, and checks `hostDiagnostics` plus the scenario musical contract. |
