...
- **macOS universal (x86_64 + arm64).** Builds both the VST3 bundle and the
  ...
  `CMAKE_OSX_ARCHITECTURES="x86_64;arm64"`. We pin the deployment target to
  11.0 so both Intel and Apple Silicon hosts stay happy, and we upload the
  bundled artifacts so testers can drag-drop without a local toolchain.
- **Linux host dependency sweep.** Installs JUCE’s usual suspects (`libx11-dev`,
  `libwebkit2gtk-4.0-dev`, etc.), configures the JUCE targets, and builds both
...
- **Windows host dependency sweep.** Leans on the Visual Studio toolchain that
  GitHub Actions ships with, layers Ninja on top, and drives the same CMake
  targets so we know JUCE + Windows stay in lockstep with the rest of the
  project.
...
- **macOS universal builds**: pass `-DCMAKE_OSX_ARCHITECTURES="x86_64;arm64"`
  ...
  list both architectures.
- **Linux builds**: if you hit missing X11/WebKit dev packages, mirror the
  `apt-get` list from the workflow and rerun CMake out-of-tree
  (`cmake -S . -B build/juce -G Ninja ...`).
- **Windows builds**: open a "x64 Native Tools" shell before running CMake to
  inherit the MSVC environment, then reuse the same flags as the workflow.
