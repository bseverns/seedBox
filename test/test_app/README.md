...
* `test_seed_lock_behaviour.cpp` proves per-seed locks survive reseeds and engine
  ...
  the snapped pitch immediately.【F:test/test_app/test_seed_lock_behaviour.cpp†L11-L74】
* `test_seed_prime_modes.cpp` focuses on the live-input prime mode, asserting we
  ...
  that engine assignments stay untouched across a reseed.【F:test/test_app/test_seed_prime_modes.cpp†L9-L41】
* `test_presets.cpp` stress-tests the preset lane promised in the roadmap by
  ...
  crossfade helper preserves granular params during the blend.【F:test/test_app/test_presets.cpp†L13-L48】
* `test_granular_source_toggle.cpp` makes sure the shift+tone UI helpers can flip
  ...
  seed in sync with what the display prints.【F:test/test_app/test_granular_source_toggle.cpp†L1-L112】
...
* The roadmap calls out tap-tempo (`recordTapTempoInterval`) and preset primes,
  ...
  legacy LFSR path without bespoke coverage.【F:docs/roadmaps/seed_system.md†L10-L24】【F:test/test_app/test_seed_prime_modes.cpp†L9-L41】
* `util::ScaleQuantizer` has `SnapUp` and `SnapDown` hooks mentioned in the doc,
  ...
  affordances.【F:docs/roadmaps/seed_system.md†L51-L57】【F:test/test_app/test_seed_lock_behaviour.cpp†L55-L74】
* Engine hand-offs are claimed to be deterministic through `EngineRouter::onSeed`,
  ...
  explicit in code.【F:docs/roadmaps/seed_system.md†L59-L70】【F:test/test_app/test_seed_lock_behaviour.cpp†L31-L43】
